	...
	<script>
		$(document).ready(function() {
			...
			function getThreads(own) {
				...
				$.get("proxy?service=getThreads&own=" + own, function(ajaxResponse){
					...
					if(ajaxResponse.length > 0 && ajaxResponse[0].error)
						...
						table = $('#example').DataTable( {
							...
							"fnRowCallback": function( nRow, aData, iDisplayIndex, iDisplayIndexFull ) {
								if ( Math.floor(aData['seen']/( '<%= request.getSession().getAttribute("userName") + " " + request.getSession().getAttribute("sn") %>' == aData['csr']? 1 : 2)) % 2 == 0  )
