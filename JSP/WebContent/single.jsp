...
<body>
			...
			<div class="container">
				<div class="row">
					...
					<div class="col-md-2 newInquiry">
						<a href="myInquiries.jsp"><button style="width: 100%;" type="button" class="btn btn-primary"><span class="glyphicon glyphicon-arrow-left" aria-hidden="true"></span>&nbsp;Return</button></a>
					</div>
					<div class="col-md-2 newInquiry" align="right">
						<form id="assignToMeForm" enctype="multipart/form-data">
							<input type="hidden" name="ownerId" value= "<%=request.getSession().getAttribute("dn") %>" >
							...
							<div class="modal-body">
								<form id="changeOwnerForm" enctype="multipart/form-data">
									...
									<div class="form-group">
			    						...
			    						<input type='text' onKeyUp="completeCsr('csrs', '#changeOwner', '#ownerOptions')" class="form-control" name="ownerId" id="changeOwner">
							...
							<div class="modal-body">
								<form id="changeStatusForm" enctype="multipart/form-data">
									...
									<div class="form-group">
										...
										<%= "<script>window.type='"+request.getSession().getAttribute("type")+"';</script>" %>
	...
	<script>
		...
		function getMessages() {
			$.get( '<%= "proxy?service=getMessages&threadId="+request.getParameter("id") %>', function( data ) {
				...
				if(!message[0] || !message[0].csr) {
					if( <%= (request.getSession().getAttribute("type").equals("csrs") || request.getSession().getAttribute("type").equals("supervisros") ? "true" : "false") %> )
						...
						customHandler: function(data) {
							...
							$.each(data, function(i, entry) { // this $.each is not included inside the $.mWidget implementation, if needed, it can be added like shown here. We know it will not allways be necessary.
						        if(entry.isSystemMessage != 0){
							        ...
							        entry.align = 'center';
						        } else if( entry.author == '<%= request.getSession().getAttribute("userName") + " " + request.getSession().getAttribute("sn") %>' ) {
								...
								if(entry.attachments){
									...
									entry.attachments.split(',').forEach(function(e){
										res += '<a href="proxy?service=getAttachment&threadId='+message[0].id+'&messageId='+entry.id+'&attachment='+escape(e)+'" class="attachments">'+e+'</a>, ';									
									})
